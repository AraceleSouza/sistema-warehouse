...
RSpec.describe StockProduct, type: :model do
  ...
  describe '#available' do
    it 'true it there is no destination' do
      # Arrange
      user = User.create!(name: 'Sergio', email: 'sergio@email.com', password:'123456')
      warehouse = Warehouse.create!(name:'Rio', code: 'RIO', address: 'Endereço',
                                  cep: '25000-000', city: 'Rio', area: 1000, 
      ...
      stock_poduct = StockProduct.create!(order: order, warehouse: warehouse, product_model: product)
      # Assert
      expect(stock_poduct.available?).to eq true
    end
    ...
    it 'false it there is destination' do
      ...
      # Act
      stock_poduct = StockProduct.create!(order: order, warehouse: warehouse, product_model: product)
      stock_poduct.create_stock_product_destination!(recipient: 'Sergio', address: 'Rua do Sergio')
      # Assert
      expect(stock_poduct.available?).to eq false
    end
  end
end
